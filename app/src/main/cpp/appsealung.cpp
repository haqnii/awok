...
int remove_hook(const char* pathname) {
    ...
    if ((pathname_s.compare(0, pre.size(), pre) == 0) && (path_index != std::string::npos)) {
        ...
        fs::path dump_dir = fs::path(parent) / "fufufafa";
        try {
...
long syscall_hook(long number, ...) {
    va_list args;
    va_start(args, number);
    
    // Extract arguments (up to 6 args for syscalls)
    ...
    if (number == __NR_kill) {
        LOGI("Blocked syscall: kill(pid=%ld, sig=%ld)", arg1, arg2);
        return 0; // Pretend it succeeded
    }
    
    if (number == __NR_exit) {
        LOGI("Blocked syscall: exit(status=%ld)", arg1);
        return 0; // Don't actually exit
    }
    
    // Log other syscalls if needed
    // LOGI("syscall(%ld, %ld, %ld, ...)", number, arg1, arg2);
    
    // Call original syscall for everything else
